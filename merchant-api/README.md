---
description: >-
  Vue d'ensemble de l'API Merchant Intram v1 — solde, payouts, transferts,
  demandes de paiement, refunds, webhooks signés.
---

# Merchant API v1 — Overview

L'**API Merchant** est l'interface server-to-server pour piloter votre compte Intram depuis votre backend : lire le solde, déclencher des payouts, transférer vers des wallets clients, demander un paiement à un client, rembourser une transaction, recevoir des notifications signées.
...
* **Asynchrone par défaut** — les mutations renvoient `202 Accepted` avec un `operation_id` que tu peux suivre par polling ou webhook signé.
...
* **Webhooks signés** — chaque event est livré avec un en-tête `X-Intram-Signature` et retentée selon un backoff exponentiel.
...
| `POST` | `/transfers` | Transfert wallet marchand → wallet client | Async (202) |
...
| `GET` | `/transfers/:reference` | Statut d'un transfert | Synchrone |
...
1. Lis l'[Authentification](authentication.md) — c'est la marche la plus haute.
2. Garde-toi 5 minutes pour [Sandbox vs Live](sandbox-vs-live.md) — c'est ce qui décide si ta requête tombe ou pas en production.
3. Suis le [Quickstart](quickstart.md) — un appel `curl` signé à `/balance` en moins de 2 minutes.
4. Active les [Webhooks](webhooks.md) — c'est comme ça que tu sauras quand un payout, un transfert ou un paiement aboutit.
